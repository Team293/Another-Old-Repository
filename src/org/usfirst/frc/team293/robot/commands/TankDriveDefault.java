...
//    	this.stickCooker(left, right, outleft, outright);
    	right = outright;
    	left = outleft;
...
//    	Robot.TrainofDriving.encoderDrive(-1*(outright), -1*(outleft));
    ...
    void stickCooker(double vleft, double vright, double voutleft, double voutright) {
    	...
    	if ( Math.abs(vright) < in1 ) {
    		voutright = vright * (out1 / in1 ); // low gain range
    	} else { 
    		voutright = rs * (out1 + ( (rs * vright)- in1 )*(1-out1)/(1-in1));
    	}
    	// Same as left side
    	if ( Math.abs(vleft) < in1 ) {
    		voutleft = vleft * (out1 / in1 ); // low gain range
    	} else { 
    		voutleft = ls * (out1 + ( (ls * vleft)- in1 )*(1-out1)/(1-in1));
    	}
