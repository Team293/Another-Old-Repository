...
public class autoLogger extends Subsystem {
	...
	String [] names = {"time","joy_left","joy_right","yaw",
			"yaw_rate","enc_left","enc_right"};
	String [] units = {"sec","defl","defl","deg","deg/s","in","in"};
	double [] dataChans = new double[7];
    ...
    public void writeFileLine(){
    	...
    	dataChans[3] = Robot.imu.getAngleX(); // yaw angle
    	dataChans[4] = Robot.imu.getRateX(); // yaw rate
    	dataChans[5] = Robot.TrainofDriving.leftEncoder.getDistance(); 
    	dataChans[6] = Robot.TrainofDriving.rightEncoder.getDistance(); 
